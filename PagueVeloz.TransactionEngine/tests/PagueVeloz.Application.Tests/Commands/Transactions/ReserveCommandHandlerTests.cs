...
public class ReserveCommandHandlerTests
{
    ...
    [Fact]
    public async Task Handle_ShouldReserveAmount_WhenAccountExistsAndAmountIsValid()
    {
        ...
        result.ReservedBalance.Should().Be(200);
        result.AvailableBalance.Should().Be(300);
        result.ErrorMessage.Should().BeNull();

        VerifyLoggerInformation("Processing reserve operation", Times.Once());
        VerifyLoggerInformation("Reserve operation completed successfully", Times.Once());
    }
    ...
    public async Task Handle_ShouldThrowNotFoundException_WhenAccountDoesNotExist()
    {
        ...
        var command = new ReserveCommand(accountId, 100, "REF-123", "BRL", null);
        ...
        VerifyLoggerInformation("Processing reserve operation", Times.Once());
        VerifyLoggerInformation("Reserve operation completed successfully", Times.Never());
    }

    [Fact]
    public async Task Handle_ShouldReturnFailedOperation_WhenReserveFailsDueToInsufficientBalance()
    {
        ...
        result.ErrorMessage.Should().Contain("insufficient available balance");
